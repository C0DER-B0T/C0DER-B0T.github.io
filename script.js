...
let data = JSON.parse(localStorage.getItem('favsData')) || {
  ...
  function renderTabs() {
    ...
    data.layouts.forEach((lyt, idx) => {
      ...
      btn.onclick = () => { data.currentLayout = idx; save(); render(); };
  ...
  function renderCards() {
    ...
    list.forEach((link, idx) => {
      ...
      card.className = 'card';
      card.innerHTML = `
        <button class="remove-btn" title="Remove this website">-</button>
  ...
  document.getElementById('toggleRemoveBtn').onclick = () => { removeMode = !removeMode; render(); };
  ...
  document.getElementById('siteForm').onsubmit = e => {
    ...
    if (name && url) {
      ...
      save(); render(); toggleModal('siteModal'); e.target.reset();
    }
  };
  ...
  document.getElementById('layoutForm').onsubmit = e => {
    ...
    if (name) {
      ...
      save(); render(); toggleModal('layoutModal'); e.target.reset();
    }
  };
  ...
  document.getElementById('exportBtn').onclick = () => {
    ...
    a.href = url; a.download = 'fav-webs-data.json'; a.click();
